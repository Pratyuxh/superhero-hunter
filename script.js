...
const publicKey = "87809d3a3d8d8c85d0cfe6c1a1341e13";
const url =
    ...
    if (unfavButton) {
      for (let button of unfavButton) {
        button.addEventListener(
          "click",
          (e) => {
            let index = favouriteMovieList.indexOf(
...
// home page content 
  ...
  if (savedLocalFavData !== null && savedLocalFavData.length > 0) {
    favouriteMovieList = savedLocalFavData;
    data.forEach((element) => {
...
function handleFav(e, item) {
  ...
  if (index === -1) {
    favouriteMovieList.push(e.target.previousElementSibling.innerText);
    window.localStorage.setItem("favData", JSON.stringify(favouriteMovieList));
